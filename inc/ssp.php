...
class SSP {
        ...
        for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
            ...
            for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
                ...
                if ( isset( $column['formatter'] ) ) {
                    $row[ $column['dt'] ] = ($isJoin) ? $column['formatter']( $data[$i][ $column['field'] ], $data[$i] ) : $column['formatter']( $data[$i][ $column['model'] ], $data[$i] );
                }
                else {
                    $row[ $column['dt'] ] = ($isJoin) ? $data[$i][ $columns[$j]['field'] ] : $data[$i][ $columns[$j]['model'] ];
                }
            }
        ...
        if ( isset($request['order']) && count($request['order']) ) {
            ...
            for ( $i=0, $ien=count($request['order']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['orderable'] == 'true' ) {
                    ...
                    $orderBy[] = ($isJoin) ? $column['model'].' '.$dir : '`'.$column['model'].'` '.$dir;
                }
            }
    ...
    static function filter ( $request, $columns, &$bindings, $isJoin = false )
    {
        ...
        if ( isset($request['search']) && $request['search']['value'] != '' ) {
            ...
            for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['searchable'] == 'true' ) {
                    ...
                    $globalSearch[] = ($isJoin) ? $column['model']." LIKE ".$binding : "`".$column['model']."` LIKE ".$binding;
                }
            }
        }
        ...
        for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
            ...
            if ( $requestColumn['searchable'] == 'true' &&
                $str != '' ) {
                $binding = SSP::bind( $bindings, '%'.$str.'%', PDO::PARAM_STR );
                $columnSearch[] = ($isJoin) ? $column['model']." LIKE ".$binding : "`".$column['model']."` LIKE ".$binding;
            }
        }
    ...
    static function simple ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '', $having = '')
    {
        ...
        if($joinQuery){
            $col = SSP::pluck($columns, 'model', $joinQuery);
       ...
       $having
            ...
            $query =  "SELECT SQL_CALC_FOUND_ROWS `".implode("`, `", SSP::pluck($columns, 'model'))."`
			 FROM `$table`
			 $where
			 $extraWhere
			 $groupBy
